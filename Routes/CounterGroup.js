...
router.put("/putCounterGroup", async (req, res) => {
  try {
    ...
    console.log(value);
    let response = await CounterGroup.updateOne(
      { counter_group_uuid: value.counter_group_uuid },
      value
    );
...
router.delete("/deleteCounterGroup", async (req, res) => {
  try {
    ...
    if (counterData.length) {
      for (let counter of counterData) {
        await Counters.updateMany(
          ...
          {
            counter_group_uuid: item.counter_group_uuid.filter(
              (a) => a !== counter_group_uuid
            ),
          }
        );
      }
    ...
    if (response.acknowledged) {
      ...
      res
        .status(404)
        .json({ success: false, message: "Counter Group Not Deleted" });
  } catch (err) {
    res.status(500).json({ success: false, message: err });
  }
});
