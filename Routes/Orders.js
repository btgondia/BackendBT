...
const CallMsg = async ({ counterData = {}, WhatsappNotification = {}, value = {} }) => {
	return;
	// try {
	// 	let data = [];
	// 	let file = [];
	...
	// 		if (value.order_uuid) {
	// 			try {
	// 				let orderpdf = await fs.promises.access("./uploads/N" + (value.invoice_number || "") + "-" + (value?.order_uuid || "") + ".pdf");
	// 			} catch (err) {
	...
	// 						path: "./uploads/N" + (value.invoice_number || "") + "-" + (value?.order_uuid || "") + ".pdf",
	// 						margin: {
	// 							top: "100px",
	// 							right: "50px",
	// 							bottom: "100px",
	...
	// 		if (contact.mobile && contact?.lable?.find(a => a.type === "wa" && +a.varification)) {
	// 			let messages = [];
	...
	// 			}
	// 			if (WhatsappNotification.checkbox && value?.order_uuid) {
	// 				fs.access("./uploads/N" + (value.invoice_number || "") + "-" + (value?.order_uuid || "") + ".pdf", err => {
	// 					if (err) {
	// 						console.log(err);
	// 						return;
	// 					}
	// 					file.push("N" + (value.invoice_number || "") + "-" + (value?.order_uuid || "") + ".pdf");

	// 					messages.push({
	// 						file: "N" + (value.invoice_number || "") + "-" + (value?.order_uuid || "") + ".pdf",
	// 						sendAsDocument: true,
...
router.post("/sendPdf", async (req, res) => {
	...
	// 	if (value.order_uuid && !fs.existsSync(path)) {
	// 		try {
	// 			const browser = await puppeteer.launch({ args: ["--no-sandbox"] });
	// 			const page = await browser.newPage();
	// 			const website_url = "https://btgondia.com/pdf/" + value.order_uuid;
			...
			if (+orderStage === 4 || +orderStage === 5 || value?.item_details?.length === 0) {
				...
				if (+orderStage === 5 || value?.item_details?.length === 0) {
					...
					fs.access("./uploads/N" + (value.invoice_number || "") + "-{" + (value?.order_uuid || "") + "}.pdf", err => {
						...
						fs.unlink("./uploads/N" + (value.invoice_number || "") + "-{" + (value?.order_uuid || "") + "}.pdf", err => {
							if (err) {
								console.log(err);
								return;
							}
				...
				if (+orderStage === 4) {
					...
					let rangeDeliveryIntensive = incentiveData.filter(
						a => a.users.filter(b => b === user_delivery_intensive).length && a.type === "delivery-incentive"
					);
							...
							if (incentive_item.calculation === "qty" && incentive_item.value) {
								amt =
									...
									(value.item_details.filter(a => +a.status !== 3).length > 1
										...
										? (+value.item_details[0].b * +itemData.find(c => c.item_uuid === value.item_details[0].item_uuid).conversion || 0) +
		...
		res.json({
			...
			result: data.map(order => {
				let receipt = receiptData?.find(
					b => b.invoice_number === order.invoice_number || (b.order_uuid === order.order_uuid && b.counter_uuid === order.counter_uuid)
				);
	...
	try {
		...
		if (userData.routes.length && !userData.routes.filter(a => +a === 1).length) {
			...
			counterData = counterData.filter(
				a => userData.routes.filter(b => b === a.route_uuid).length || (userData.routes.filter(b => b === "none").length && !a.route_uuid)
			);
...
router.post("/GetOrderCheckingList", async (req, res) => {
	try {
		...
		result = data
			...
			?.filter(
				a =>
					(a.status.length > 1 ? +a.status.reduce((c, d) => Math.max(+c.stage, +d.stage)) === 2 : +a?.status[0]?.stage === 2) && a.item_details.length
			);
...
router.post("/GetOrderDeliveryList", async (req, res) => {
	try {
		...
		result = data
			...
			?.filter(
				a =>
					(a.status.length > 1 ? +a.status.map(c => +c.stage).reduce((c, d) => Math.max(c, d)) === 3 : +a?.status[0]?.stage === 3) &&
	...
	try {
		...
		response = response
			...
			.filter(
				a =>
					...
					counterData.filter(b => a.counter_uuid === b.counter_uuid && b.counter_group_uuid.filter(c => c === value.counter_group_uuid).length).length
			)
		...
		for (let a of itemsData.filter(a => !value.company_uuid || a.company_uuid === value.company_uuid)) {
			...
			let obj = {
				...
				sales_amt:
					salesData.length > 1 ? salesData.map(b => b.item_total || 0).reduce((a, b) => +a + b) : salesData.length ? salesData[0].item_total || 0 : 0,
			...
			deliver_return_percentage: (
				Math.abs(((+a.deliver_returnB * (+a.conversion || 1) || 0) + a.deliver_returnP) * 100) /
					(+a.salesB * (+a.conversion || 0) +
						...
						(a.processing_canceledB * a.conversion + a.processing_canceledP) || 1) || 0
			).toFixed(2),
