...
const createCreditNotAccountingVoucher = async (order, type, narration) => {
	...
	for (let item of order.item_details) {
		if (item.css_percentage) css_percentage = item.css_percentage + css_percentage
	}
...
const createAccountingVoucher = async ({
	...
	let isGst = gst?.startsWith("27") || !gst ? false : true
	let arr = []
	const gst_value = Array.from(new Set(order.item_details.map(a => +a.gst_percentage)))
	...
	if (order?.replacement || order?.shortage || order?.adjustment) {
		await createAutoCreditNote(
			order,
			gst ? "7605d5e9-8165-46aa-8899-5c2d40622d30" : "d68051cc-573d-4721-b42e-bd226157268b",
			voucher_date
		)
	}
	let total = 0
	...
	for (let value of req.body) {
		...
		if (+new_stage === 4 || +new_stage === 5 || value?.item_details?.length === 0) {
			...
			if (+new_stage === 4) {
				...
				let user_delivery_intensive = status.find(c => +c.stage === 4)?.user_uuid
