	...
	try {
		...
		if (response) {
			...
			if (value?.counter_charges?.[0]) {
				...
				const updated_data = { status, invoice_number: `${value?.order_type}${_invoice_number}` }
			...
			if (+orderStage === 4 || +orderStage === 5 || value?.item_details?.length === 0) {
				...
				if (+orderStage === 4) {
					...
					let rangeItemIntensive = incentiveData.filter(
						a => a.users.filter(b => b === user_range_order).length && a.type === "item-incentive"
					)
...
router.put("/order_datetime", async (req, res) => {
	try {
		...
		const result = await Orders.updateOne({ order_uuid: data.order_uuid }, data)
	...
	try {
		...
		response = response.map(order => ({
			...
			cash: receiptData
				...
				?.modes?.find(b => b.mode_uuid === "c67b54ba-d2b6-11ec-9d64-0242ac120002")?.amt,
			cheque: receiptData
				.find(b => b.order_uuid === order.order_uuid)
				?.modes.find(b => b.mode_uuid === "c67b5794-d2b6-11ec-9d64-0242ac120002")?.amt,
			upi: receiptData
				.find(b => b.order_uuid === order.order_uuid)
...
router.post("/getOrderItemReport", async (req, res) => {
	try {
		...
		response = response
			...
			.filter(
				a =>
					...
					counterData.filter(
						b => a.counter_uuid === b.counter_uuid && b.counter_group_uuid.filter(c => c === value.counter_group_uuid).length
					).length
			)
		...
		let FinalData = data.map(a => ({
			...
			deliver_return:
				+a.deliver_returnB + parseInt(+a.deliver_returnP / +a.conversion) + ":" + (+a.deliver_returnP % +a.conversion),
			processing_canceled:
			...
			auto_added_percentage: (
				((a.auto_addedB * a.conversion + a.auto_addedP) * 100) / (a.salesB * a.conversion + a.salesP || 1) || 0
			).toFixed(2),
			deliver_return_amt: (Math.abs(a.sales_amt * (+a.conversion * +a.deliver_returnB + a.deliver_returnP)) || 0).toFixed(2),
			processing_canceled_amt: (a.sales_amt * (+a.conversion * (+a.processing_canceledB + a.processing_canceledP)) || 0).toFixed(
				2
			),
