...
router.get("/getCounterSales/:days", async (req, res) => {
	try {
		...
		orderData = orderData?.filter(
			order => order.status.filter(a => +a.stage === 1 && a.time > time).length
		);
		let data = await Counter.find({});
		data = JSON.parse(JSON.stringify(data));
				...
				let orderItems = [].concat
					...
					?.map(
						a =>
							+(
								+a.b +
								+(ItemsData?.find(b => b.item_uuid === a.item_uuid)?.conversion || 0) +
								+a.p
							) * +a.price
					);
				let value =
		...
		for (let counter of counterData) {
			...
			if (average_lines.length) {
				await Counter.updateMany(
					...
					{
						[type === "company" ? "average_lines_company" : "average_lines_category"]:
							average_lines,
					}
				);
			}
