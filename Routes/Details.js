...
router.post("/mobileOrderSequence", async (req, res) => {
	try {
		const { mobile_order_sequence } = req.body
		let data = await Details.updateOne({}, { mobile_order_sequence })
		if (data?.acknowledged) {
			let details = await Details.findOne({})
			res.json({ success: true, result: details.mobile_order_sequence })
		} else res.json({ success: false, message: "Details Not found" })
	} catch (err) {
		res.status(500).json({ success: false, message: err })
	}
